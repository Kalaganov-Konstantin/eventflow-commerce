...
func LoadConfig() (*Config, error) {
	...
	loader := config.New("payment")
	loader.SetDefault("server.host", "0.0.0.0")
	loader.SetDefault("kafka.group_id", "payment-service")
	loader.SetDefault("kafka.dlq_topic", "payments.events.dlq")
	...
	loader.SetDefault("service.name", "payment")
	...
	if err := loader.BindEnv("server.port", "PAYMENT_SERVER_PORT", "PAYMENT_SERVICE_PORT"); err != nil {
		return nil, fmt.Errorf("failed to bind server.port: %w", err)
	}
	if err := loader.BindEnv("database.url", "PAYMENT_DATABASE_URL"); err != nil {
	...
	if dbURLString == "" {
		return nil, fmt.Errorf("PAYMENT_DATABASE_URL environment variable is not set")
	}
	cfg.DatabaseURL = dbURLString

	parsedURL, err := url.Parse(dbURLString)
	if err != nil {
		return nil, fmt.Errorf("invalid PAYMENT_DATABASE_URL: %w", err)
	}
...
func (c *Config) Validate() error {
	...
	if c.Server.Port == "" {
		return fmt.Errorf("PAYMENT_SERVER_PORT (or PAYMENT_SERVICE_PORT) environment variable is not set")
	}
	...
	if c.Database.Host == "" {
		return fmt.Errorf("database host is required in PAYMENT_DATABASE_URL")
	}
	if c.Database.Port == "" {
		return fmt.Errorf("database port is required in PAYMENT_DATABASE_URL")
	}
	if c.Database.User == "" {
		return fmt.Errorf("database user is required in PAYMENT_DATABASE_URL")
	}
	if c.Database.Password == "" {
		return fmt.Errorf("database password is required in PAYMENT_DATABASE_URL")
	}
	if c.Database.DBName == "" {
		return fmt.Errorf("database name is required in PAYMENT_DATABASE_URL")
	}
